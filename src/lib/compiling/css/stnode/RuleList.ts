/** 80**************************************************************************
 * @module lib/compiling/css/stnode/RuleList
 * @license MIT
 ******************************************************************************/
...
/** @final */
export class RuleList extends CSSSn {
  readonly #rc_a: RCSnt[];
  ...
  override replaceChild(oldSn_x: RCSn, newSn_x?: RCSn | undefined): void {
    const i_ = this.#rc_a.indexOf(oldSn_x);
    if (i_ < 0) return;

    this.children$ = undefined;

    const atBdry = i_ === 0 || i_ === this.#rc_a.length - 1;
    if (newSn_x) {
      newSn_x.attachTo_$(this);
      this.#rc_a.splice(i_, 1, newSn_x);
    } else {
      this.#rc_a.splice(i_, 1);
    }
